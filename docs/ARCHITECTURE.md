...
`lumic.toml` schema version 2 is the versioned repository-to-server contract and uses the same application, capability, typed configuration, resource, runtime-extension, process, schedule, deployment, and health vocabulary as the trusted built-in catalog. `lumic-core` owns its strict transport-neutral schema, validation, and capability resolution. `lumic-platform` performs bounded non-symlink reads from the repository root and keeps inspection, planning, and approved application separate. During deployment, the manifest from the exact checked-out commit is authoritative for source/public paths and deployment phases; unresolved service requirements block the release. Repository manifests cannot declare privileged implementation paths or arbitrary commands. Legacy `lumic.yaml` schema version 1 remains readable during migration, but the two files may not coexist.
