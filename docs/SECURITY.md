...
MySQL/PostgreSQL/Redis reference settings are provider-allowlisted and limited to loopback. Database identifiers and resource IDs are validated before direct-argv native execution. Generated database passwords come from `/dev/urandom`, are stored in mode-`0600` files, are passed to the native SQL client on stdin and are represented in state/UI/CLI/MCP/audit only by secret reference. Sensitive resource outputs must use the `secret://` reference form; application database bindings never embed the credential value. Material configuration writes are atomic and record both backups and newly created paths for rollback after restart/health failure.
