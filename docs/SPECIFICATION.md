...
Implemented reference direction: MySQL, PostgreSQL, Redis, Typesense, Meilisearch, Valkey, RabbitMQ, MinIO, OpenSearch, Memcached, MongoDB, ClickHouse, Prometheus, Grafana, and Loki use allowlisted native Debian/Ubuntu apt packages, systemd, loopback-only validated configuration, provider health/log hooks, and persistent state/events. MySQL, PostgreSQL, and Redis additionally implement local backup/restore. Trusted Rust drivers referenced by built-in catalog definitions own their platform mapping, validation/rendering, paths, health, supported backup/restore plans, and child-resource commands. The compatibility manager executes those typed plans without closed provider dispatch, and persists through the schema-v2 resource store while preserving its existing CLI/UI/MCP contract. Relational database/user/grant resources publish database and sensitive credential-reference outputs; applications can bind multiple role-owned database pairs after grants are recorded. Typesense and Meilisearch generate private service credentials and publish reusable HTTP endpoint plus sensitive credential-reference outputs; application bindings protect a consumed search service from removal. Configured third-party apt sources remain an explicit operator prerequisite for external packages. The ten newer drivers reject backup/restore and child-resource operations until those provider contracts are implemented. nginx is independently installed and persisted as a singleton managed service; its owned web-host resources use atomic validated configuration and explicit bindings. PHP runtimes are versioned resources that publish deterministic FPM/CLI outputs and own version-qualified extension components. The reviewed WordPress recipe implements a checksum-pinned, idempotent PHP/MySQL/nginx lifecycle with durable operation state, rollback, secret references, health, and safe removal through the shared verified artifact manager. Application lifecycle plans expose package requirements only after policy review and identify the source from which trust was derived. The fixed software-installer catalog separately exposes package setup through UI and MCP. NVM uses an explicit account, a pinned official Git tag and managed profile activation; other installer entries remain native package setup rather than managed-provider configuration.

Gitea and Gogs additionally use pinned SHA-256-verified upstream artifacts, dedicated system users, hardened systemd units, and the configured Lumic repository root through a shared group. Only one forge may own a root, and filesystem sharing does not imply automatic forge-database registration.
...
The implemented webhook uses HTTPS with a public DNS/IP destination, a secret reference, HMAC-SHA256 headers, a 256 KiB envelope bound, per-destination timeout, exponential bounded retry and retained outcome history. Delivery re-resolves and validates every address, pins the approved address, bypasses proxy configuration and permits only HTTPS redirects. Subscriptions match exact event/entity filters. The reference rule handles `service.failed` with a validated `.service` target, cooldown, at most three attempts and `active_state=active` verification. Rule/destination configuration follows plan/apply and retains a rollback snapshot. MCP additionally requires an explicit process scope and per-call approval.
...
Candidate recipes after primitives are mature: Laravel, Symfony, WordPress, Drupal, Forgejo, Matomo/analytics apps and other commonly self-hosted software.
