#!/bin/bash
##### TODO COPY TO STACKSCRIPTS/ lumic.sh
...
DB_NAME='lumicdb'
DB_PASSWORD=$(openssl rand -base64 32|sha256sum|base64|head -c 32| tr '[:upper:]' '[:lower:]')
DBUSER='lumicuser'
DBUSER_PASSWORD=$(openssl rand -base64 24|sha256sum|base64|head -c 32| tr '[:upper:]' '[:lower:]')

sudo pam-auth-update --package
sudo mount -o remount,rw /
sudo chmod 640 /etc/shadow
sudo useradd -m -s /bin/bash cipi
echo "cipi:cipipass"|sudo chpasswd
...
APP_TIMEZONE=UTC
ROOT_USER_NAME=${MYSQL_ROOT_USER}
ROOT_USER_PASS=${MYSQL_ROOT_PASS}
...
# Stackscript cleanup
###########################################################
echo "Cleaning up..." >> /var/www/html/status.txt
rm /root/StackScript
rm /root/ssinclude*
