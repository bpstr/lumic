...
| Applications | versioned strict `lumic.yaml` repository contract for static/PHP/Node runtime, source/public paths, argv-only build/migrate hooks, workers, cron, service requirements, health and deployment intent; application-scoped authenticated-encrypted environment values with masked MCP inspection, key-only diff, controlled set/rotate/delete and deployment-time injection; native Git push dispatch gated by the contract; immutable releases, deploy locks/cancellation, pinned retry/redeploy, commit metadata/log cursors, health-gated rollback, blue/green Node nginx handoff and drain, explicit apt-available PHP 8.1–8.4 runtimes/extensions, resource-bound Certbot/Let's Encrypt TLS | generic certificate mutation controls in every adapter, third-party PHP repositories, shared release paths |
...
| Infrastructure | namespaced group-shared managed bare Git repositories, external discovery/registration/adoption, explicit remotes/fetch/push, authenticated Smart HTTP, Gitea/Gogs installers sharing the configured repository root, portable environment references, two-node identity/trust, signed deploy/rollback envelopes and restricted-SSH MCP access | per-repository identities and SSH Git transport; Forgejo managed driver and automatic forge metadata reconciliation; central fleet UI |
